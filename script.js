...
function saveRequestQueryPromise(name) {
  return getBreedByStringPromise(name).then((res) => {
...
function addBreedToFav(event) {
  ...
  saveRequestQueryPromise(event.target.textContent).then((obj) => {
...
$(document).ready(() => {
  ...
  getAllBreedsPromise().then((breedsObjectsArray) => {
  ...
  searchBreedBtn.addEventListener("click", (event) => {
    ...
    getBreedByStringPromise(userBreedInput.value).then((breedInfo) => {
